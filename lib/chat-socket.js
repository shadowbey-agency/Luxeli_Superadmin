...
export function initChatSocket() {
  ...
  // Get user info for logging (only in development)
  ...
  // Note: Backend uses JWT token role for authentication (not userData from localStorage)
  // This is correct and secure - token role is the source of truth
      ...
      if (error && typeof error === 'object') {
        if (Object.keys(error).length > 0) {
          ...
          if (error.message) {
            ...
            // Backend configuration errors (like missing models) are backend issues
            if (error.message.includes('model not found') || error.message.includes('Model')) {
              console.error('   ⚠️ This is a backend configuration issue. Please check the chat server backend.');
            }
          }
        }
      } else if (error) {
